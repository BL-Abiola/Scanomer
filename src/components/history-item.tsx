...
export const HistoryItem = ({ item, onClick }: { item: AnalysisResult; onClick: () => void }) => {
  ...
  return (
    <button
      onClick={onClick}
      className={cn(
        "w-full rounded-xl border-l-4 bg-white/5 p-3 sm:p-4 text-left transition-all hover:bg-white/10 hover:ring-2 hover:ring-primary/50",
        colorClasses.border
      )}
    >
      <div className="flex items-center justify-between">
        <div className="flex min-w-0 items-center gap-3 sm:gap-4">
          ...
          <div className="min-w-0">
            ...
            <p className="truncate text-xs sm:text-sm text-muted-foreground">{item.description}</p>
          </div>
        </div>
